...
class Stationary(Kernel):
    ...
    def __init__(self, input_dim, variance=0.1, lengthscales=1.0,
                 active_dims=None, ARD=None, name=None, U_kernel_optimization = False):
        """
        ...
        self._v = variance
        self._l = lengthscales
        self.logvariance = tf.Variable(np.log(variance), dtype=tf.float64, name='log_variance', trainable=U_kernel_optimization)
        self.variance = tf.exp(self.logvariance)

        lengthscales, self.ARD = self._validate_ard_shape("lengthscales", lengthscales, ARD)
        self.loglengthscales = tf.Variable(np.log(lengthscales), dtype=tf.float64, name='log_lengthscales', trainable=U_kernel_optimization)
        self.lengthscales = tf.exp(self.loglengthscales)
    ...
    def _scaled_square_dist(self, X, X2):
        ...
        dist += Xs + tf.linalg.matrix_transpose(X2s)
        return dist
    ...
    def __str__(self):
        str = [
            ...
            ' Variance = %.3f' % self._v,
            ' Lengthscales = %s (ARD = %s)' % (np.array2string(np.array(self._l), precision=3), self.ARD),
        ]
        return '\n'.join(str)
...
class LinearK(Kernel):
    ...
    def __init__(self, input_dim, variance=1.0, active_dims=None, ARD=None, name=None):
        ...
        self.logvariance = tf.Variable(np.log(variance), dtype=tf.float64, name='log_variance', trainable=False)
        self.variance = tf.exp(self.logvariance)
