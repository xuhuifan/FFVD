...
class Stationary(Kernel):
    ...
    def __init__(self, input_dim, variance=0.1, lengthscales=1.0,
                 active_dims=None, ARD=None, name=None, kernel_optimization = False):
        """
        ...
        self.logvariance = tf.Variable(np.log(variance), dtype=tf.float64, name='log_variance', trainable=kernel_optimization)
        self.variance = tf.exp(self.logvariance)

        lengthscales, self.ARD = self._validate_ard_shape("lengthscales", lengthscales, ARD)
        self.loglengthscales = tf.Variable(np.log(lengthscales), dtype=tf.float64, name='log_lengthscales', trainable=kernel_optimization)
